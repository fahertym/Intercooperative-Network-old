...
impl Blockchain {
    pub fn new(consensus: Arc<Mutex<Consensus>>, sharding_manager: Arc<Mutex<ShardingManager>>) -> Self {
    ...
    pub fn add_transaction(&mut self, transaction: Transaction) -> Result<(), String> {
        ...
        if from_shard == to_shard {
            ...
            drop(sharding_manager); // Release the lock before calling process_cross_shard_transaction
            self.sharding_manager.lock().unwrap().process_cross_shard_transaction(transaction)
        }
    }
    ...
    pub fn validate_transaction(&self, transaction: &Transaction) -> Result<(), String> {
        let sharding_manager = self.sharding_manager.lock().map_err(|_| "Failed to acquire lock on sharding manager")?;
        let balance = sharding_manager.get_balance(transaction.from.clone(), transaction.currency_type.clone());
...
mod tests {
    ...
    fn test_add_transaction() {
        ...
        blockchain.sharding_manager.lock().unwrap().add_address_to_shard("Alice".to_string(), 0);
        blockchain.sharding_manager.lock().unwrap().add_address_to_shard("Bob".to_string(), 0);
        blockchain.sharding_manager.lock().unwrap().initialize_balance("Alice".to_string(), CurrencyType::BasicNeeds, 1000.0);
